...
class PolymarketClient:
    ...
    def fetch_leaderboard(
        self, category="OVERALL", time_period="DAY", order_by="PNL", limit=25
    ):
    ...
    def fetch_closed_positions(
        self, user=None, limit=100, sort_by="REALIZEDPNL", sort_direction="DESC"
    ):
        ...
        if user is None:
            ...
            for trader_address in leaderboard["proxyWallet"].head(5).to_list():
                try:
                    url = self.builder.closed_positions(
                        trader_address,
                        limit=20,
                        sort_by=sort_by,
            ...
            return (
                pl.DataFrame(all_positions[:limit]) if all_positions else pl.DataFrame()
            )

        url = self.builder.closed_positions(
            user, limit, sort_by, sort_direction
        ).build()
        response = requests.get(url)
        response.raise_for_status()
        return pl.DataFrame(response.json())
    ...
    def fetch_activity(
        self, user=None, limit=100, sort_by="TIMESTAMP", sort_direction="DESC"
    ):
            ...
            for trader_address in leaderboard["proxyWallet"].head(5).to_list():
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        all_activity.extend(
                            activity if isinstance(activity, list) else [activity]
                        )
                except Exception:
                    continue

            return (
                pl.DataFrame(all_activity[:limit]) if all_activity else pl.DataFrame()
            )
    ...
    def fetch_all(
        ...
        return {
            ...
            "closed_positions": self.fetch_closed_positions(
                user=user_for_details, limit=positions_limit
            ),
            "trades": self.fetch_trades(limit=trades_limit),
            "activity": self.fetch_activity(
                user=user_for_details, limit=activity_limit
            ),
        }
